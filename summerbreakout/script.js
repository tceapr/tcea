const locks = [
  {
    ...
    title: 'Word Lock',
    badge: 'Word',
    label: 'No spaces or commas. Example: WAVE',
    answerHash: 'cb90ab28'
  },
  {
    id: 'colorFive',
  ...
  },
  {
    ...
    badge: 'Direction',
    label: 'No spaces or commas. Example: DLRU',
    answerHash: '7e86c298'
  },
  {
    id: 'numberFour',
...
function updateProgress() {
  ...
  if (solved === locks.length && !winDialog.open && !winQueued) {
    winQueued = true;
    window.recordSuccessfulSolve?.('summer-sunshine-breakout');
...
function startLeafCelebration() {
  ...
  for (let i = 0; i < 44; i += 1) {
    const sunshine = document.createElement('span');
    sunshine.className = 'summer-sun';
    sunshine.style.setProperty('--x', `${Math.random() * 100}vw`);
    sunshine.style.setProperty('--drift', `${Math.random() * 220 - 110}px`);
    sunshine.style.setProperty('--duration', `${4.8 + Math.random() * 2.8}s`);
    sunshine.style.setProperty('--delay', `${Math.random() * 1.4}s`);
    sunshine.style.setProperty('--spin', `${Math.random() * 680 + 240}deg`);
    leafCelebration.appendChild(sunshine);
  }
